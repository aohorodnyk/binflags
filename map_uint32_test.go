...
func TestHasFlagMapUint32(t *testing.T) {
	...
	for _, prov := range providerHasFlagMapUint32() {
		prov := prov
		t.Run(prov.name, func(t *testing.T) {
			t.Parallel()

			actual := binflags.HasFlagMapUint32(prov.flags, prov.flag)
...
func TestSetFlagMapUint32(t *testing.T) {
	...
	for _, prov := range providerSetFlagMapUint32() {
		prov := prov
		t.Run(prov.name, func(t *testing.T) {
			t.Parallel()

			err := binflags.SetFlagMapUint32(prov.flags, prov.flag, prov.set)
...
func providerHasFlagMapUint32() []providerTypeHasFlagMapUint32 {
	return []providerTypeHasFlagMapUint32{
		...
		},
		{
			...
			flags:    map[uint64]uint32{0: 1},
			flag:     0,
			expected: true,
		},
		{
			name:     "Flags {0: 32768}, flag 15",
			flags:    map[uint64]uint32{0: 32768},
			flag:     15,
			expected: true,
		},
		{
			name:     "Flags {0: 2147483648}, flag 31",
			flags:    map[uint64]uint32{0: 2147483648},
			flag:     31,
			expected: true,
		},
		{
			name:     "Flags {0: 2147483648, 1: 1}, flag 32",
			flags:    map[uint64]uint32{0: 2147483648, 1: 1},
			flag:     32,
			expected: true,
		},
		{
			name:     "Flags {0: 2147483648, 1: 1449509}, flag 52",
			flags:    map[uint64]uint32{0: 2147483648, 1: 1449509},
			flag:     52,
			expected: true,
		},
		{
			name:     "Flags {0: 2147483648, 1323: 1449509}, flag 42356",
			flags:    map[uint64]uint32{0: 2147483648, 1323: 1449509},
			flag:     42356,
			expected: true,
		},
	}
	...
	return []providerTypeSetFlagMapUint32{
		{
			...
			flag:     0,
			set:      true,
			expected: map[uint64]uint32{0: 1},
		},
		{
			...
			flag:     7,
			set:      true,
			expected: map[uint64]uint32{0: 128},
		},
		{
			...
			flag:     15,
			set:      true,
			expected: map[uint64]uint32{0: 32768},
		},
		{
			name:     "Flags {}, flag 7, set",
			flags:    map[uint64]uint32{0: 92},
			flag:     7,
			set:      true,
			expected: map[uint64]uint32{0: 220},
		},
		{
			name:     "Flags {0: 220}, flag 7, unset",
			flags:    map[uint64]uint32{0: 220},
			flag:     7,
			set:      false,
			expected: map[uint64]uint32{0: 92},
		},
		{
		...
		},
		{
			...
			flag:     14,
			set:      true,
			expected: map[uint64]uint32{0: 16604},
		},
		{
			name:     "Flags {0: 16604}, flag 31, set",
			flags:    map[uint64]uint32{0: 16604},
			flag:     31,
			set:      true,
			expected: map[uint64]uint32{0: 2147500252},
		},
		{
			name:     "Flags {0: 2147500252}, flag 14, unset",
			flags:    map[uint64]uint32{0: 2147500252},
			flag:     14,
			set:      false,
			expected: map[uint64]uint32{0: 2147483868},
		},
		{
			name:     "Flags {0: 2147500252}, flag 5234, set",
			flags:    map[uint64]uint32{0: 2147500252},
			flag:     5234,
			set:      true,
			expected: map[uint64]uint32{0: 2147500252, 163: 262144},
		},
		{
			name:     "Flags {0: 436324, 1: 1, 2: 25623541}, flag 32, unset",
			flags:    map[uint64]uint32{0: 436324, 1: 1, 2: 25623541},
			flag:     32,
			set:      false,
			expected: map[uint64]uint32{0: 436324, 2: 25623541},
		},
		{
